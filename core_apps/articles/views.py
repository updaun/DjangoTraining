...
class ArticleRetrieveUpdateDestroyView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def perform_update(self, serializer):
        ...
        if "banner_image" in self.request.FILES:
            if (
                ...
                and instance.banner_image != "/profile_default.png"
            ):
                default_storage.delete(instance.banner_image.path)
            instance.banner_image = self.request.FILES["banner_image"]
            instance.save()
        logger.info(
            f"article {serializer.data.get('title')} updated by {self.request.user.first_name}"
        )
    ...
    def perform_destroy(self, instance):
        instance.delete()
        logger.info(
            f"article {instance.title} deleted by {self.request.user.first_name}"
        )
